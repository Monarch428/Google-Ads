...
function buildIntroMessage(mode: ChatbotMode, clientName?: string) {
  if (mode === "account") {
    if (clientName) {
      return `You're chatting in the context of ${clientName}. I'll analyze their recommendations, expected impact, and next actions.`;
    }
    return "Switch to account-specific chat by selecting a client. I'll use their recommendations to tailor guidance.";
  }

  return "Hi! I'm your AI Recommendations assistant. Ask me to summarize insights, prioritize actions, or bundle steps across accounts.";
}
...
export function AIRecommendationsChatbot() {
  ...
  const generalSuggestions = [
    ...
    "Bundle actions to move the needle fastest",
  ];

  const accountSuggestions = [
    "Summarize this client's top recommendations",
    "Which actions will drive the biggest lift for this account?",
    "Create an action bundle for this client's next sprint",
    "What blockers should I fix first for this account?",
  ];
          ...
          {mode === "account" && (
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-xs text-slate-500">
                Select a client account to ground responses in their recommendations and data.
              </p>
              <Select
